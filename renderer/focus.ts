...
import type { Analysis, Impact, ImpactEntry, RefMeta } from '../src/semantics.ts';
...
export const Focus = (() => {
  ...
  const meta = (r: Ref): RefMeta => need(SEM().refMeta[r.id] ?? null, `refMeta[${r.id}]`);
  ...
  function junctionTag(other: string, focus: string): string {
  ...
  function odcChip(r: Ref): string {
    const od = meta(r).onDelete;
  ...
  function actClause(r: Ref): string {
    const m = meta(r), od = m.onDelete, ou = m.onUpdate;
    const WORDS = {
  ...
  function relLine(r: Ref): string {
    const m = meta(r), ty = SEM().TYPES[m.type];
  ...
  let lastImpact: Impact | null = null; // 영향권 모드 중 현재 포커스의 deleteImpact 결과

  function toggleImpact(): void {
    ST().impact = !ST().impact;
    const f = ST().focusTable;
    if (f) void render(f);
  }

  // ── 2-hop 미리보기(.fx) — 카드 형제 인라인 확장, 수동 peek은 동시 1개 ──
  function onwardCount(other: string): number {
  ...
  function buildPeek(name: string): HTMLElement {
    ...
    for (const p in nb.parents) if (p !== ST().focusTable)
      for (const r2 of nb.parents[p] ?? []) rows.push({ r: r2, dir: '▶', other: p, ti: Math.max(0, TYPE_ORDER.indexOf(meta(r2).type)) });
    for (const c in nb.children) if (c !== ST().focusTable)
      for (const r2 of nb.children[c] ?? []) rows.push({ r: r2, dir: '◀', other: c, ti: Math.max(0, TYPE_ORDER.indexOf(meta(r2).type)) });
    rows.sort((a, b) => a.ti - b.ti || a.other.localeCompare(b.other));
    const shown = rows.slice(0, 6);
    panel.innerHTML = shown.map(({ r: r2, dir, other }) => {
      const m = meta(r2), ty = SEM().TYPES[m.type];
    ...
    panel.querySelectorAll<HTMLElement>('.fxrow').forEach((b) =>
      b.addEventListener('click', () => { const g = b.dataset['goto']; if (g) cb?.go(g); }));
    return panel;
  }

  function closePeek() {
    const open = document.querySelector<HTMLElement>('#focuswrap .fitem > .fx');
    if (!open) return false;
    const card = open.parentElement?.querySelector<HTMLElement>('.fnode');
    const b = card?.querySelector<HTMLElement>('.fxbtn');
    if (b) b.setAttribute('aria-expanded', 'false');
    open.remove();
    return true;
  }

  function togglePeek(card: HTMLElement, focusFirst?: boolean): void {
    if (ST().impact) return; // 영향권 모드의 .fx 슬롯은 체인이 소유(모드 파생·다중) — 수동 peek 비활성
    const wrap = card.parentElement;
    if (!wrap || !wrap.classList.contains('fitem')) return;
    const wasOpen = !!wrap.querySelector<HTMLElement>('.fx');
    ...
    if (focusFirst) {
      const r = panel.querySelector<HTMLElement>('.fxrow');
  ...
  function openPeek(name: string): boolean {
    const card = document.querySelector<HTMLElement>(`#focuswrap .fnode[data-name="${CSS.escape(name)}"]`);
  ...
  function nodeCard(other: string, edges: Ref[]): HTMLElement {
    ...
    elB.addEventListener('click', (e) => { if ((e.target as Element | null)?.closest('button')) return; cb?.go(other); });
    ...
    if (rows.length >= 2) rows.forEach((sp) => {
      sp.tabIndex = -1;
      sp.addEventListener('focus', () => setHot(other, elB.closest('#fcolR') ? (sp.dataset['col'] ?? null) : null, true));
      sp.addEventListener('blur', () => setHot(null, null, false));
    });
    const skip = elB.querySelector<HTMLElement>('.fskip');
    if (skip) skip.addEventListener('click', () => cb?.go(far));
    const fxb = elB.querySelector<HTMLElement>('.fxbtn');
    if (fxb) fxb.addEventListener('click', () => togglePeek(elB, false));
    return elB;
  }

  function focusCard(t: Table): HTMLElement {
    const gv = ST().groupColor[t.group ?? ''] || '--gc-x';
    const div = document.createElement('div');
    div.className = 'ffocus'; div.style.setProperty('--gc', `var(${gv})`);
    const passesFilter = (r: Ref): boolean => ST().filter !== 'real' || r.kind === 'real';
    ...
    for (const c of colsShown) {
      ...
      let cell;
      if (ref) {
        const m = meta(ref), ty = SEM().TYPES[m.type];
    ...
    if (ST().impact && lastImpact) {
      ...
      const cnt = (fn: (x: Impact['vetoed'][number]) => boolean): number => v.filter(fn).length;
    ...
    div.querySelector<HTMLElement>('.fimpact')?.addEventListener('click', toggleImpact);
    div.tabIndex = -1; // 키보드 홈 위치
    div.querySelectorAll<HTMLElement>('.fk[data-goto]').forEach((a) =>
      a.addEventListener('click', (e) => { e.stopPropagation(); const g = a.dataset['goto']; if (g) cb?.go(g); }));
    ...
    div.querySelectorAll<HTMLElement>('.col-row').forEach((row) => {
      ...
      row.tabIndex = -1;
      row.addEventListener('pointerenter', () => setHot(null, col ?? null, true));
      row.addEventListener('pointerleave', () => setHot(null, null, false));
      row.addEventListener('focus', () => setHot(null, col ?? null, true));
  ...
  function stationsOf(colEl: HTMLElement): HTMLElement[] {
    ...
    for (const el of colEl.querySelectorAll<HTMLElement>('.fnode:not(.fmore-hidden), button.fmore')) {
      ...
      if (el.classList.contains('fnode')) {
        ...
        const fx = el.parentElement?.querySelector<HTMLElement>('.fx');
        if (fx) fx.querySelectorAll<HTMLElement>('.fxrow').forEach((r) => out.push(r));
      }
    }
    return out;
  }
  const focusEl = (el: HTMLElement | null | undefined): boolean => {
    if (!el) return false;
  ...
  function onKey(e: KeyboardEvent): boolean {
    ...
    if (k === ' ') {
      const card = inWrap ? a?.closest<HTMLElement>('.fnode') : null;
    ...
    if (k === 'ArrowUp' || k === 'ArrowDown') {
      e.preventDefault();
      const home = $('fcolM').querySelector<HTMLElement>('.ffocus');
      if (!inWrap) return focusEl(home); // 첫 진입은 중앙 홈
      const side = a?.closest<HTMLElement>('#fcolL') ?? a?.closest<HTMLElement>('#fcolR');
      let list: HTMLElement[];
      if (side) list = stationsOf(side);
      else {
        list = home ? [home, ...home.querySelectorAll<HTMLElement>('.col-row[tabindex]')] : [];
      }
      const idx = a instanceof HTMLElement ? list.indexOf(a) : -1;
      if (idx === -1) return focusEl(list[0] ?? home);
      return focusEl(list[idx + (k === 'ArrowDown' ? 1 : -1)] ?? (a instanceof HTMLElement ? a : null)) || true;
    }
    ...
    if (k === 'ArrowLeft' || k === 'ArrowRight') {
      ...
      if (!inL && !inR) { // 중앙 — 와이어 따라가기
        if (k === 'ArrowRight') {
          const col = a instanceof HTMLElement ? a.dataset['col'] : undefined;
          if (col) for (const name in parentsEdges) {
            if ((parentsEdges[name] ?? []).some((r2) => r2.child.cols[0] === col)) {
              const tgt = document.querySelector<HTMLElement>(`#fcolR .fnode[data-name="${CSS.escape(name)}"]`);
              if (tgt) return focusEl(tgt);
            }
          }
          return focusEl($('fcolR').querySelector<HTMLElement>('.fnode')) || true;
        }
        return focusEl($('fcolL').querySelector<HTMLElement>('.fnode')) || true;
      }
      if (inL && k === 'ArrowRight') {
        // 좌열 → 중앙: drawWires가 쓰는 정박 행(rep.parent.cols[0])으로 역산
        const edges = childrenEdges[(card as HTMLElement | null)?.dataset['name'] ?? ''] ?? [];
        const rep = edges.find((r2) => r2.kind === 'real') ?? edges[0];
        const row = rep ? $('fcolM').querySelector<HTMLElement>(`.col-row[data-col="${CSS.escape(rep.parent.cols[0] ?? '')}"]`) : null;
        return focusEl(row || home);
      }
      if (inR && k === 'ArrowLeft') {
        const edges = parentsEdges[(card as HTMLElement | null)?.dataset['name'] ?? ''] ?? [];
        const e0 = edges[0];
        const row = e0 ? $('fcolM').querySelector<HTMLElement>(`.col-row[data-col="${CSS.escape(e0.child.cols[0] ?? '')}"]`) : null;
        return focusEl(row || home);
      }
      // 바깥 방향(좌열 ←, 우열 →) = .fx 확장 토글
      if (card instanceof HTMLElement && ((inL && k === 'ArrowLeft') || (inR && k === 'ArrowRight'))) {
    ...
    if (k === 'Enter') {
      ...
      const card = a?.closest<HTMLElement>('.fnode');
      const cn = card?.dataset['name'];
      if (cn) { cb?.go(cn); return true; }
      if (a.classList.contains('col-row')) {
        const link = a.querySelector<HTMLElement>('.fk[data-goto]');
        const g = link?.dataset['goto'];
        if (g) cb?.go(g);
        return true;
      }
  ...
  function chainRow (en: ImpactEntry, level: number) {
    ...
    b.style.setProperty('--lv', String(level - 2));
    ...
    b.title = `${en.table} — ${Semantics.ACTIONS[en.action]?.label ?? en.action}${en.guaranteed ? '' : ' (앱 레벨 — DB 보장 없음)'}${en.warning ? ' · ' + en.warning : ''}`;
    b.addEventListener('click', () => { const g = b.dataset['goto']; if (g) cb?.go(g); });
    return b;
  }
  function countDesc (entries: ImpactEntry[]) {
    let n = 0;
    (function w(l) { for (const e of l) { n++; w(e.children || []); } })(entries);
    return n;
  }
  function appendChain (panel: HTMLElement, entries: ImpactEntry[], level: number) {
    for (const en of entries) {
      ...
      if (level < 2) appendChain(panel, en.children, level + 1);
        ...
        badge.style.setProperty('--lv', String(level - 1));
  ...
  function fillImpact (colEl: HTMLElement, impact: Impact) {
    ...
    const CATS = [
      { label: '연쇄 삭제', cssVar: '--act-cascade', test: (en: ImpactEntry) => en.kind === 'real' && en.action === 'cascade' },
      { label: 'NULL 전환', cssVar: '--act-setnull', test: (en: ImpactEntry) => en.kind === 'real' && en.action === 'set null' && !en.veto },
      { label: '기본값 전환', cssVar: '--act-setnull', test: (en: ImpactEntry) => en.kind === 'real' && en.action === 'set default' },
      { label: '삭제 차단', cssVar: '--act-restrict', test: (en: ImpactEntry) => en.kind === 'real' && en.veto && en.specified },
      { label: '기본 동작(미지정 — 행이 있으면 차단)', cssVar: '--act-restrict', soft: true, test: (en: ImpactEntry) => en.kind === 'real' && en.veto && !en.specified },
      { label: '앱 레벨(비보장) — 필터와 무관하게 표시', cssVar: '--logical', soft: true, test: (en: ImpactEntry) => en.kind === 'logical' && !en.orphan && en.specified },
      { label: '고아 가능(논리·표기 없음)', cssVar: '--logical', soft: true, test: (en: ImpactEntry) => en.kind === 'logical' && en.orphan },
    ];
    for (const cat of CATS) {
      ...
      for (const [tbl, list] of byTable) {
        const card = nodeCard(tbl, list.map((en: ImpactEntry) => refById.get(en.refId)).filter((r: Ref | undefined): r is Ref => !!r));
        card.dataset['refids'] = list.map((en: ImpactEntry) => en.refId).join(','); // 와이어 정박을 섹션 소유 ref로 한정
        const wrap = document.createElement('div');
        wrap.className = 'fitem';
        wrap.appendChild(card);
        const kids = list.flatMap((en: ImpactEntry) => en.children || []);
        if (kids.length) {
          const panel = document.createElement('div');
  ...
  function pt(elm: Element, stRect: DOMRect) {
  ...
  function setHot(other: string | null, col: string | null, on: boolean): void {
    const wires = $('fwires');
    ...
    if (on && other) {
      const n = document.querySelector<HTMLElement>(`#focuswrap .fnode[data-name="${CSS.escape(other)}"]`);
      if (n) n.classList.add('hot');
    }
    const fe = $('fcolM').querySelector<HTMLElement>('.ffocus');
    if (fe) {
      fe.querySelectorAll<HTMLElement>('.col-row.hot').forEach((r2) => r2.classList.remove('hot'));
      if (on && col) {
        const r2 = fe.querySelector<HTMLElement>(`.col-row[data-col="${CSS.escape(col)}"]`);
  ...
  function drawWires() {
    ...
    const focusEl = $('fcolM').querySelector<HTMLElement>('.ffocus');
    if (!focusEl) return;
    const st = stage.getBoundingClientRect();
    const F = pt(focusEl, st);
    const fcolsEl = focusEl.querySelector<HTMLElement>('.f-cols');
    const FC = fcolsEl ? pt(fcolsEl, st) : F;
    const rowY = (colName: string | null | undefined): number | null => {
      const row = colName && focusEl.querySelector<HTMLElement>(`.col-row[data-col="${CSS.escape(colName)}"]`);
      return row ? clamp(pt(row, st).cy, FC.t + 6, FC.b - 6) : null;
    };
    const mkNS = (tag: string, attrs: Record<string, string | number>): SVGElement => {
      const e = document.createElementNS(NS, tag) as SVGElement;
      for (const k in attrs) e.setAttribute(k, String(attrs[k]));
      return e;
    };
    // 관계(엣지) 하나 = 와이어 하나. 기본은 중립색, hover/hot 시 유형색으로 강조(dbdiagram 문법).
    // info: 이 와이어가 어느 이웃·어느 컬럼에 걸린 것인지 — 상호 하이라이트에 쓴다
    const mk = (a: number, b2: number, ay: number, by: number, ref: Ref, info: { other: string; col: string }, cls?: string): void => {
      const m = meta(ref), cvar = SEM().TYPES[m.type].cssVar;
      ...
      hit.addEventListener('pointerenter', (e: PointerEvent) => {
      ...
      hit.addEventListener('pointermove', (e: PointerEvent) => { if (cb?.tooltip) cb.tooltip.move(e.clientX, e.clientY); });
    ...
    $('fcolL').querySelectorAll<HTMLElement>('.fnode').forEach((nd) => {
      ...
      if (nd.dataset['refids']) {
        ...
        edges = edges.filter((e2: Ref) => ids.has(e2.id));
      }
      if (!edges.length) return;
      const b = pt(nd, st);
      const rep = edges.find((e: Ref) => e.kind === 'real') ?? edges[0];
      if (!rep) return;
    ...
    $('fcolR').querySelectorAll<HTMLElement>('.fnode').forEach((nd) => {
      ...
      const b = pt(nd, st);
      edges.forEach((edge: Ref, i: number) => {
  ...
  async function render(name: string): Promise<void> {
    ...
    if (prevA && $('focuswrap').contains(prevA)) {
      ...
      else if (prevA instanceof HTMLElement && prevA.dataset['ref']) cursor = { kind: 'row', ref: prevA.dataset['ref'] };
      else if (prevA.classList.contains('col-row')) cursor = { kind: 'colrow', col: (prevA as HTMLElement).dataset['col'] ?? '' };
      else {
        const c = prevA.closest<HTMLElement>('.fnode');
    ...
    const TYPE_ORDER = Object.keys(SEM().TYPES);
    type Grouped = { other: string; edges: Ref[]; ti: number };
    const fillLeft = (colEl: HTMLElement, map: Record<string, Ref[]>): void => {
      const list: Grouped[] = Object.keys(map).map((o) => {
        const edges = map[o] ?? [];
        const first = edges[0];
        return { other: o, edges, ti: first ? Math.max(0, TYPE_ORDER.indexOf(meta(first).type)) : 0 };
      ...
      const sections: { ti: number; items: Grouped[] }[] = [];
      for (const it of list) {
        let cur = sections[sections.length - 1];
        if (!cur || cur.ti !== it.ti) { cur = { ti: it.ti, items: [] }; sections.push(cur); }
        cur.items.push(it);
      }
      for (const sec of sections) {
        const ty = SEM().TYPES[(TYPE_ORDER[sec.ti] ?? 'ref') as keyof Analysis['TYPES']];
        const hd = document.createElement('div');
    ...
    const fillRight = (colEl: HTMLElement, map: Record<string, Ref[]>): void => {
      const list = Object.keys(map).map((o) => {
        const edges = map[o] ?? [];
        return {
          other: o, edges,
          ri: Math.min(...edges.map((r) => { const i = colIdx.get(r.child.cols[0] ?? ''); return i == null ? 999 : i; })),
        };
    ...
    return new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => {
      ...
      const f = M.querySelector<HTMLElement>('.ffocus');
      ...
      if (cursor) {
        ...
        if (cursor.kind === 'home') el = f;
          ...
          el = document.querySelector<HTMLElement>(`#focuswrap .nrel > span[data-ref="${CSS.escape(cursor.ref ?? '')}"]`);
        else if (cursor.kind === 'colrow')
          el = M.querySelector<HTMLElement>(`.col-row[data-col="${CSS.escape(cursor.col ?? '')}"]`);
        else if (cursor.kind === 'card')
          el = document.querySelector<HTMLElement>(`#focuswrap .fnode[data-name="${CSS.escape(cursor.name ?? '')}"]`);
        const target = el || f;
